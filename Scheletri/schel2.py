...
def falsi(fname, a, b, maxit, tolx,tolf):
    ...
    while  abs(b-a) >= tolx and it < maxit:
        ...
        if math.copysign(1,fa)*math.copysign(1,fxk)>0:   
            a = xk
            fa = fname(xk)
        elif math.copysign(fxk)*math.copysign(fb)>0:    
            b = xk
            fb = fname(xk)
...
def my_newtonSys(fun, jac, x0, tolx, tolf, nmax):
  ...
  while it < nmax and npl.norm(fx1, 1) >= tolf and np.linalg.norm(s, 1) >= np.linalg.norm(x1,1)*tolx:
    x0 = x1
    it += 1
    matjac = jac(x0)
    if npl.det(matjac) == 0:
...
def my_newton_minimo_MOD(gradiente, Hess, x0, tolx, tolf, nmax):
  ...
  while it < nmax and npl.norm(s, 1) > tolx * npl.norm(x1, 1) and npl.norm(grad_fx1, 1) > tolf:
...
def jacobi(A,b,x0,toll,it_max):
    ...
    raggiospettrale= max(autovalori)
