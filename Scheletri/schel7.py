...
def metodo_bisezione(fname, a, b, tolx,tolf):
  ...
  while it < maxit and abs(b-a) >= tolx:  #to do
    xk =  (a+b)/2
    v_xk.append(xk)
    it += 1
    fxk=fname(xk)
...
def newton(fname,fpname,x0,tolx,tolf,nmax):
  ...
  while  abs(fx1) >= tolf and it < nmax and abs(d) >= tolx*abs(x1) :
    x0= x1
    fx0= fname(x0)
    if fpname(x0) <= np.spacing(1): #Se la derivata prima e' pià piccola della precisione di macchina stop
      print(" derivata prima nulla in x0")
      return None, None,None
    d= fx0/fpname(x0)
...
def my_newtonSys(fun, jac, x0, tolx, tolf, nmax):
  ...
  while npl.norm(fx1, 1) >= tolf and it < nmax and npl.norm(s, 1) >= tolx*npl.norm(x1, 1):
    x0 = x1
    it += 1
    matjac = jac(x0)
    if npl.det(matjac) == 0:
...
def my_newton_minimo(gradiente, Hess, x0, tolx, tolf, nmax):
  ...
  while npl.norm(grad_fx1, 1) >= tolf and it < nmax and npl.norm(s, 1) >= tolx*npl.norm(x1, 1):
